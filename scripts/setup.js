...
readline.question(`Please provide the FaunaDB admin key\n`, (adminKey) => {
  ...
  streamToPromise(stream)
    ...
    .then((res) => {
      // The GraphQL schema is important, this means that we now have a BloodDonorEntry Collection and an blood_donor_entries index.
      ...
      return client
        .query(
          q.CreateRole({
            ...
            privileges: [
              {
                resource: q.Collection('BloodDonorEntry'),
                actions: { read: true, write: true, create: true },
              },
              {
                resource: q.Index('blood_donor_entries'),
                actions: { read: true },
              },
            ],
          })
        ...
        .then((res) => {
          console.log('2. Successfully created role to read and write BloodDonor entries')
        })
        .catch((err) => {
